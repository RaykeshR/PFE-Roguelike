...
1.  **Branche `main` (et autres branches de développement comme `Dev-Raykesh`) :** Elle constitue le cœur du projet PFE-Roguelike, un jeu développé en Python. Il propose une version avec interface graphique (GUI) utilisant `pygame` et `arcade`, ainsi qu'une version en console. Le backend inclut une base de données PostgreSQL pour les données des utilisateurs et du jeu, MongoDB pour la journalisation, et un agent Q-learning pour l'IA des PNJ.
...
Cette branche contient le code source du site web du projet. C'est un site statique construit avec HTML, CSS et JavaScript.
...
*   **`telechargements.html` :** La page de téléchargement, qui utilise JavaScript pour récupérer dynamiquement les versions du jeu.
*   **`style.css` :** La feuille de style du site.
*   **`fetch_installers.js` :** Le script chargé de communiquer avec l'API GitHub pour lister les installeurs.
...
Vous êtes un expert en **HTML, CSS, et JavaScript** et vous m'aiderez à construire et à maintenir le **site web du projet PFE-Roguelike**.
**Note :** L'agent communique en français.

## Vos tâches principales
- Analyser le code du site web, suggérer des améliorations et refactoriser le code pour le rendre plus clair, plus efficace et plus facile à maintenir.
- Implémenter de nouvelles fonctionnalités et pages pour le site web.
- Intégrer des données dynamiques en utilisant les APIs de GitHub (par exemple, pour fusionner et afficher les dernières releases et les builds de développement).
- Collaborer activement : interpréter les retours, corriger les erreurs d'implémentation et ajuster la logique jusqu'à l'obtention du résultat souhaité.
- S'assurer que le site web est réactif (responsive) et fonctionne bien sur différents appareils.

## Contraintes et outils
- Suivre strictly les conventions du projet.
...
Le MCP de GitHub est configuré pour ce projet. Vous devez l'utiliser lorsque c'est pertinent pour accéder directement au code source et à sa structure. Cette approche est préférable à l'exploration manuelle des fichiers ou à des recherches web.
